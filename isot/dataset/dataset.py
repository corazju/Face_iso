...
from isot.utils import to_tensor, to_list
from isot.utils.output import prints, Indent_Redirect
...
class Dataset:
    ...
    def get_dataset(self, mode: str, full: bool = True, classes: List[int] = None, **kwargs) -> torch.utils.data.Dataset:
        if full and mode != 'test':
            ...
            dataset = subset[mode]
        if classes:
            dataset, _ = self.get_class_set(dataset, classes=classes, **kwargs)
        return dataset
    ...
    def get_class_set_extra(self, dataset: torch.utils.data.Subset, classes: List[int], label_list: List[int], **kwargs):
        indices = np.arange(len(dataset))
        idx_bool = np.isin(label_list, classes)
        idx = np.arange(len(dataset))[idx_bool]
        idx = np.intersect1d(idx, indices)
        return torch.utils.data.Subset(dataset, idx),idx

    
    def get_index_set(self, dataset: torch.utils.data.Dataset, indexes: List[int], **kwargs):
        
        return torch.utils.data.Subset(dataset, indexes)
